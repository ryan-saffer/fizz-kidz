...
- Birthday-party instruction groups read package names and colours from Sanity and derive creation instructions through each customer package's creations. Staff-only packages such as Sweet Kitty own a direct instruction list.
- The custom [party-form checkout](src/features/party-bookings/core/party-form-v2/README.md) prepares server-priced Square orders and accepts embedded card, discount-code and gift-card payments through tRPC. Only paid submissions can update the booking.
- The booking catalogue includes active and retired creations. Each live creation owns its operational studio/mobile availability, which applies in every package. Current Paperform values resolve within their package by stable key, current name, or legacy label; a unique archived value such as Nutella Slime resolves globally for delayed or replayed historical submissions. A package-and-channel-specific transition map temporarily handles the copied mobile Science list only for live mobile-capable creations. Unknown or unavailable values are logged and rejected. Booking writes validate new creation keys against their party channel while allowing an existing historical selection to remain unchanged.
