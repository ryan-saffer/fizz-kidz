...
- The booking catalogue includes active and retired creations. Each live creation owns its operational studio/mobile availability, which applies in every package. Paperform values resolve within their package by stable key, current name, or legacy label. A package-and-channel-specific transition map temporarily handles the copied mobile Science list only for live mobile-capable creations, plus the exact archived Nutella option still present in studio Slime. Unknown or unavailable values are logged and rejected. Booking writes validate new creation keys against their party channel while allowing an existing historical selection to remain unchanged.
