...
The server renders a pending state with only "let's" and a purple cursor visible, preventing a flash of the finished gold word before typing. The homepage preloads the shared Lilita font through the layout's `head` slot. Startup waits only for Lilita, not `document.fonts.ready`, decorative fonts or React hydration. A hidden static word is revealed by CSS for reduced motion and by a noscript fallback when JavaScript is disabled.
