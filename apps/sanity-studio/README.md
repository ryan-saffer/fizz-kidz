...
Use **Birthday Parties > Reorder and publish packages** to insert or move a package. Choose its new position and save once; the Studio renumbers every affected package as drafts, so positions do not need to be changed or published individually. Package documents are published from this screen rather than from the document editor. The **Publish all package drafts** button validates and publishes every package draft in one atomic action. Sanity's existing direct Netlify webhook still receives one event per changed document, even when the documents are published together.

The catalogue migration and post-deployment field cleanup are complete. Production Sanity contains 11 active packages, 56 live creations, 66 archived historical creations, and no migration markers, duplicate package fields, card-level availability, per-card booking order, or old instruction references. The completed one-time migration scripts were removed; Git history retains the audit trail.
...
The production **Build Website** document webhook listens for published creates, updates, and deletes of `birthdayPartyPackage`, `birthdayPartyCreationOffering`, `holidayProgramWeek`, and `websiteImage` documents. Draft and Content Release version edits do not trigger builds. Because it is a filtered document webhook, publishing several changed documents together may send more than one Netlify build request.
