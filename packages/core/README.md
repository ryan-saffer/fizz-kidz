...
`src/parties/birthday-party-catalogue.ts` defines the normalized party-package catalogue contract. Each package has one canonical name, a primary colour, an accent colour, one Website position, and one card sequence; exactly one card per creation carries booking channels and booking order. Website position controls the menu, Party Themes, and all-creations catalogue order. One shared colour-options array defines every supported package colour, label, and CSS value for both colour fields and their consumers. Pure helpers derive the `Parties`, `Creations`, and `Party Package` labels used by consumers. Packages also carry their generated Website route, SEO, hero, navigation, Party Themes card, and optional feature sections. The validator checks package and creation identity, booking configuration, routes and positions, presentation data, image metadata, and legacy-label ambiguity. Sanity clients remain in their owning apps and return this runtime-neutral shape.
