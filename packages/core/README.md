...
`src/parties/birthday-party-catalogue.ts` defines the normalized party-package catalogue contract. Each package has one card sequence; exactly one card per creation carries booking channels and booking order. The validator checks package and creation identity, booking configuration, presentation data, image metadata, and legacy-label ambiguity. Sanity clients remain in their owning apps and return this runtime-neutral shape.
