...
`src/parties/birthday-party-catalogue.ts` defines both the full Website catalogue and a smaller booking catalogue. Each package has one canonical name, a primary colour, an accent colour, one position, and one card sequence. Website card order is also booking-menu order; repeated cards for one creation are deduplicated at their first occurrence. Each creation owns its operational studio/mobile availability regardless of how many packages or cards use it. Position controls Portal instruction groups and the Website menu, Party Themes, and all-creations catalogue order. The booking contract and pure helpers validate package choices, filter them by creation availability, resolve current or legacy Paperform labels to stable creation keys, and retain archived creations for historical display. Unknown historical keys remain unchanged and display as their raw stable key. Sanity clients remain in their owning apps and return these runtime-neutral shapes.
