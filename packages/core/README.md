...
`src/parties/birthday-party-catalogue.ts` defines the normalized party-package catalogue contract. Each package has one card sequence; exactly one card per creation carries booking channels and booking order. Packages also carry their generated Website route, SEO, hero, navigation, Party Themes card, and optional feature sections. The validator checks package and creation identity, booking configuration, routes and listing orders, presentation data, image metadata, and legacy-label ambiguity. Sanity clients remain in their owning apps and return this runtime-neutral shape.
