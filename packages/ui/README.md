...
The fonts use lossless WOFF2 compression, retaining the original glyphs, metrics and hinting. The original TTF/OTF files remain the conversion sources. Regenerate a font with `uv run --with 'fonttools[woff]' fonttools ttLib.woff2 compress path/to/font.otf`.

The Lilita font is also exported at `@fizz-kidz/ui/assets/fonts/LilitaOne.woff2` so consumers can preload the same asset for critical headings. Vite consumers can import that path with `?url`. The TTF export remains available for existing consumers.
